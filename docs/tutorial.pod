...
If you prefer the original "cuddled-else" style you can
...
4 spaces.  You may change this to be a different number with B<-i=n>.
...
line with the command B<-et=n>, where B<n> is the number of spaces which
will be represented by one tab.
...
an output filename by appending an extension, F<.tdy>, to whatever
filename it is given.  On most systems, you may use wildcards to
reformat a whole batch of files at once, like this for example:
...
should make regular additional backups to other media as well.  
...
returned by the C<perl -c> check will be discarded by default, but it
will be included in the error output file if the B<-w> flag is given.
The reason for discarding the message by default is that C<perl -c> can
