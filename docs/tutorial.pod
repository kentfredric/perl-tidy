...
with a B<-ce> flag.  So if you rerun with that flag
...
Another noticable difference between the original and the reformatted
...
must NOT be adjacent to the flag, like this B<-i3> B(WRONG).  Also,
...
editors, and in the long run, will avoid maintenance problems.  (It may
be hard to appreciate this unless you have had to remove tabs from
someone else's program, and guarantee that the program still functions
exactly as before).  
...
of spaces which will be represented by one tab.
...
the input file with the extension F<.tdy> appended.
...
One thing that you cannot do with perltidy is to reformat a script
and overwrite the original version.  This is a safety precaution.

=head2 Making Backups

In an actual project, you will need to make a backup copy of the
...
RCS directory, and a current separate backup as F<testfile.pl.bak>.  Of
course, you should make regular additional backups to other media as
well.  

Please do not fall into this trap:  Let's say you are working on a
...
a F<.tdy> file, your work will be lost if you inadvertently run perltidy
again on the original script.  To avoid this, you need to rename your
files before starting to work again.
...
Let's perform a fire drill to see how perltidy reports errors.  Try
...
The default is generally a good choice, but if you are trying to
locate a bug, you might find the B<-w> flag useful.

=head2 Syntax Checking

Perltidy will normally try to check itself by running C<perl -c> on both
...
If the input file has an error according to the C<perl -c> check, then
the output file will not be checked, and any error messages returned by
the C<perl -c> check will be discarded (because they usually turn out to
be more annoying than helpful).  If you want to see them, though, you
can use the B<-w> flag.  This might be useful if you are debugging a
script.
...
Here's a diagram showing how this works:
     ...
     check input file ---------->  check output file ---------> ok
             ...
             | Fail                       | Fail
             |                            |
       -w shows errors             perltidy made error


This syntax check will be deactivated for Windows 9x/Me and also when
...
create a relatively sparce log file, use
