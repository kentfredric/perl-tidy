...
Some limited testing on two different Windows machines gave these
discouraging results using Active State Perl:

    Binary Build    Perl Version     Result
    522             5.005_03         perltidy works ok
    623             5.6.0            Windows hangs
    628             5.6.1            Windows hangs

I would be interested in hearing of other successes or failures in
running perltidy under Windows.  Here is how to install perltidy
under windows.
...
perltidy (and perl) under Windows with the standard F<command.com>
shell.  The problem is that F<command.com> always sends the standard
...
It also creates an empty file named F<&1>.  This means that you will
have to check for syntax errors yourself.   If you do not want to create
the empty file F<&1>, you can turn off syntax checking with the flag
B<-nsyn>.  You can then check the syntax of a file manually by issuing the
command

 perl -c filename

The best way to solve this problem is probably to use a better shell,
such as bash - see http://www.cygwin.com.  If anyone knows a way
to code around this in perl please let me know.
...
Release 20010608 had a bug with the -t option in which a tab
...
the -chk option to see if this might have happened.
