...
=head2 More options for controling placement of opening/closing tokens

Many have requested even more options to control opening and closing token
placement.  
...
=head2 improved internal if/unless formatting

Consider giving internal if/unless statements an additional level
of indentation.  This would avoid running out of indentation levels.
...
=head2 enable -ole under Windows

This only works under unix (or cygwin) at present.  It doesn't work for Windows
versions, such as Active State, because they change line endings that they
don't like.
