...
=head2 2001 xx xx
 ...
 -Lines with a single ?/: pair will now be more likely to break at both
 the ? and the :
 ...
 discussions and examples.  There are two main reasons for this.  First,
 leading ':'s can be aligned and more easily seen.  Second, a ':' is
 essentially equivalent to an 'else' or 'elsif', which is also at the
 start of a line.  
 ...
 -Paranoia struck.  Added -T flag when doing perl -c syntax check.  This
 is because I run a large number of scripts from sources unknown, and who
 knows what might be hidden in initialization blocks?  Also, deactivated 
 the syntax check if perltidy is run as root.
