...
=head2 2002 xx xx
 ...
 overwrite the original file with the beautified output file.  The original
 file will be saved with a '.bak' extension (which can be changed with
 -bext=s).  Please see updated docs.  Thanks to Rudi Farkas for the suggestion
 and example coding.
 ...
 A separate flag could be made for this, but since most cuddled-else
 users will also want cuddled-continues, no new flag has been
 introduced.  Suggested by Simon Perreault.  
 ...
 perltidy was run.  Thanks to Mike Birdsall for reporting this.
