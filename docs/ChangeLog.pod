=head1 Perltidy Change Log
 ...
 -A new -w (--warning-output) flag has been implemented and documented
  ...
  NOTE: This replaces the undocumented -w=0 or --warning-level flag
 ...
 -New syntax check: after wasting 5 minutes trying to resolve a syntax
  ...
  I spent a few more minutes adding a check for this in perltidy.  

 -The behavior of --break-before-subs (-bbs) and --break-before-blocks
 (-bbb) has been modified slightly, hopefully for the better.  Also, a
 new control parameter, --long-block-line-count=n (-lbl=n) has been
 introduced to give more control on -bbb.  This was previously a
 hardwired value.  These are documented in the man page.  The reason for
 the change is to reduce the number of blank lines that perltidy
 introduces, and make it less erratic.  I did a lot of experimenting
 with different schemes for introducing blank lines before code blocks,
 and decided that there is no really sane way to do it.  I think the new
 scheme is an improvement, though.  The idea is to avoid long sections
 without any breaks at all in that code you inherited from Bubba.
 Thanks to Erik Thaysen for bringing this to my attention.
 ...
 -I discovered a mis-tokenization of $#-.  Some experimenting with Perl
 indicated that it allows just these non-alphanumeric '$#' array maximum
 index varaibles: $#: $#- $#+ so I updated the parser accordingly.  (You
 can use the @; array, for example, but you cannot get at its maximum
 index with $#;  ).  Now perltidy can parse something like this correctly:
           $#=+$#:+ +$#-+$#++$#;#$#:)
 ...
 -Discovered a new bug involving during testing!  See BUGS.html.  
