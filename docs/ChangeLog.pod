...
=head2 2006 06 06
 ...
 and not formatted.  Thanks to Borris Zentner, Greg Ferguson, Steve Kirkup.
 ...
 The default is to only do this for certain keywords. 
 ...
 -The -gnu flag was changed from
        lp bl noll pt=2 bt=2 sbt=2 cpi=1 csbi=1 cbi=1
   to:
        lp bl noll pt=2 bt=2 sbt=2 cpi=2 csbi=1 cbi=1
   This seems to be better overall, but to recover the old format
   you can use -gnu -cpi=1
 ...
 -Temporary new flags -sot (stack-opening-token), -sct (stack-closing-token)
 are in the code for Tony.  They are not documented yet because it is likely
 that they will change, so it is best to avoid using them until they are
 finalized.
