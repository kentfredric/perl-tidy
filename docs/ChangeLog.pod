...
=head2 2001 xx xx
 ...
 Added -html control over pod text, using a new abbreviation 'po'.  See
 ...
 .po to use this.  The old color was the color of a string, and there
 was no control.  

 Long lines containing '?:' conditionals will now have the ':' placed
 at the start of a line, one per line.  For example:
 ...
 discussions and examples.  The thinking is that the ':' is equivalent
 to an 'else' or 'elsif', which is also at the start of a line.  Also,
 leading ':'s can be aligned and more easily seen.
 ...
 NOTE: concatenated items which are a single token with total number of
 characters (including " marks) given by
 --short-concatenated-item-length=n (-scl=n) will not be placed on
 a new line.  The default value is n=8.  In the above example, the
 ...
 NOTE: 'good' location means before 'and','or','if','unless','&&','||'
 ...
 -Added -T flag when doing perl -c syntax check, for security.  
