...
=head2 2002 xx xx 
 ...
 -New parameter --dump-profile (or -dpro) will dump to standard output
  information about the search for a configuration file, the name of
  whatever configuration file is selected, and its contents.  This should
  help debugging config files, especially on different Windows systems.
 ...
 -Corrections added for a leading ':' and for leaving a leading 'tcsh'
  ...
  Perl will silently swallow a leading colon on line 1 of a script, and now
  perltidy will do likewise.  For example, this is a valid script, provided
  that it is the first line, but not otherwise:
 ...
 -List formatting improved for certain lists with special initial terms, such
  as occur with 'printf', 'sprintf', 'push', 'pack', 'join', 'chmod'.  The 
  special initial term is now placed on a line by itself.  For example,
  perltidy -gnu
 ...
 -Lists of complex items, such as matricies, are now detected and displayed
  with just one item per row:
 ...
 -An interrupt handler has been added to delete temporary files if
  perltidy is interrupted.

 -Perltidy will now use system-generated temporary filenames when
  necessary, rather than create files with .TMPI and .TMPO extensions.

 -Merged patch sent by Yves Orton to handle appropriate configuration file
  locations for different Windows varieties (2000, NT, XP, 95, 98).

 -Added patch to properly handle a for/foreach loop without parens
  around a list represented as a qw.  I didn't know this was possible
  until Wolfgang Weisselberg pointed it out:

        foreach my $key qw\Uno Due Tres Quadro\ {
            print "Set $key\n";
        }

  But this is probably not a very good programming practice.  Perl will 
  give a syntax error without the $ variable; ie this will not work:
 ...
 -For the -lp option, a single long term within parens (without commas) now 
  has better alignment.  For example, perltidy -gnu
