=head1 Perltidy Change Log
 ...
 have hit a bug or unusual behavior, or have a suggested improvement,
...
=head2 2001 xx xx  
 ...
 -Uploaded perltidy to cvs 2001-06-10

 -A '-gnu' flag has been added which causes lists to be indented
 with extra indentation in the manner sometimes associated with
 emacs or the GNU suggestions.  (Suggested by Ian Stuart).  NOTE:
 This feature is functional for simple cases but will be under 
 development for some time.  The formatting reverts to the standard
 method when the -gnu coding is incomplete.

 -Subroutine call parameter lists are now formatted as other lists.
 This will improve formatting of tables being passed via subroutine
 calls.  This will also cause full indentation ('-i=n) of continued
 ...
 cases.  It can be deactivated with --no-hanging-side-comments.
 (Suggested by Brad Eisenberg).
