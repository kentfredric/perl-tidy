...
=head2 2002 xx xx
 ...
 for horizontal tightness for consistency.  This way $a{'-foo'},
 $a{foo}, and $a{-foo} are now all treated similarly.  Thus, by default,
 OLD: $a{ -foo } will now be NEW: $a{-foo}.  Suggested by Mark Olesen.
 ...
 -Improved alignment of ?/: chains of the form
  OLD:
    $leapyear =
      $year % 4     ? 0
      : $year % 100 ? 1
 ...
 -All perl code distributed with perl 5.8 is parsed correctly except
 for one instance (lextest.t) of the known bug.
