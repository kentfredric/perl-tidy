...
=head2 2003 xx xx
 ...
 -The default has been changed to not do syntax checking with perl.  
  Use -syn if you really want it.  Perltidy is very robust now, and I've
  spent too much time tracking down problems in BEGIN blocks (which get
  executed with perl -c).  For example, perltidy will never return when
  trying to beautify this code if -syn is used:
  ...
  which may hang using -syn if a shared file system is unavailable.
 ...
 -Added flag '--preserve-line-endings' or '-ple' to cause the output
  ...
  or mac line endings.  Suggested by Rainer Hochschild.
