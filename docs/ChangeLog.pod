=head1 Perltidy Change Log
 ...
 was causing trouble with a regex test because of the '*' characters:
  ...
  bla bla
  *EOF*
 Perl seems to allow absolutely anything to be a here doc target, so
 an exact string comparison is now used.
 ...
 -Fixed parsing of subroutine attributes (A Perl 5.6 feature).  
