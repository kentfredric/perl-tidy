...
=head2 2001 xx xx
 ...
 to introduce this problem when cutting and pasting, and perl does not
 complain about it.  

 -To simplify development and testing of .perltidyrc files, the command
    -pro=filename  or -profile=filename may be used to specify a
  configuration file which will override the default name of
  .perltidyrc.  There must not be a space on either side of the '='
  sign.  

 -New command -acsc (--add-closing-side-comment) and several related
 commands allow comments to be added to (and deleted from) selected
 closing block braces.  This can be useful to maintenance programmers;
 see updated man page.  Thanks to Peter Masiar for this suggestion.
 ...
 -Two new commands for outdenting, -olq (outdent-long-quotes) and -olc
  ...
  the new sticky block comment option

 -Sticky block comments:  to provide a way to display commented code
  better, the convention is used that comments with a leading '##' should
  not be formatted as usual.  Please see '-sbc' (or '--sticky-block-comment')
 ...
 -Two changes were made to help show structure of complex lists:
  ...
  The following example illustrates both of these points. Thanks to
  Wolfgang Weisselberg for this snippet and a discussion of it.  Note
  how it is easier to see the call parameters in the NEW version:
    ...
    -The structure of nested logical expressions is now displayed
    better, as illustrated by this snippet taken from Deparse.pm. 
  ...
  -Corrected tokenization error for the following (non-recommended) construct:
    $user = @vars[1] / 100;
