...
not both, so it does not do any beautification when the B<-html> option
is used.  This means that any beautification must be done in a
preliminary separate run without the B<-html> switch.
...
The short L<"EXAMPLES"> section should help you get started quickly.

The L<"OPTIONS"> section describes all of the command line parameters. 
...
braces and lime, bold, italics keywords the following command would be
used:
