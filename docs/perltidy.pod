...
This causes perltidy to run C<perl -T -c> to check syntax of input and output.
The results are written to the F<.LOG> file, which will be saved if an error
is detected in the output script.  The output script is not checked if the
input script has a syntax error.  To skip syntax checking, use B<-nsyn> or
B<--nocheck-syntax>.  Syntax checking is also deactivated by the B<--quiet>
...
Certain commands are incompatable with tabs, and if these are given,
then a warning message will be given and tabs will be deactivated.  An
example is the B<-lp> option.
...
space is required.  Examples are commands for outdenting labels, B<-ola>,
and control keywords, B<-ock>.
...
=item Sticky Block Comments: B<-sbc>, B<--sticky-block-comments>

When B<-sbc> is used, a block comment with a special leading pattern will
be treated specially, and will effectively behave as if it had glue along
its left and top edges.  This option is useful for controlling
how commented code is displayed.

A sticky block comment is identified by a special leading pattern, which
by default is two (or more) leading hash symbols, C<##>.  See the next
item, B<-sbcp>, for a way to change this.

Comments so identified  are treated as follows: 
(1) If there is no leading space on the line, then the comment will not
be indented, and otherwise it may be, (2) no new blank line will be
inserted before such a comment, and (3) such a comment will never become
a hanging side comment.  For example, assuming C<@month_of_year> is
left-adjusted:
...
=item B<-sbcp=string>, B<--sticky-block-comment-prefix=string>

This parameter defines the prefix used to identify sticky block comments
...
identify all comments as sticky block comments, one would use C<-sbcp=#>.

Please note that B<-sbcp> merely defines the pattern used to identify
sticky block comments; it will not be used unless the switch B<-sbc> is
...
A closing side comment is a special comment which can be placed after
the closing brace of a code block.  They can be useful for code
maintenance and debugging.  The command B<-acsc> (or
B<-add-closing-side-comments>) adds closing side comments (if none
exists).  Here is a small code snippet which has been processed with
B<-acsc>: 
...
The result of this command is to add comments such as the last one, 
C<## end sub foo>, to closing block braces.  

The command B<-dcsc>, (or B<--delete-closing-side-comments>) reverses
this process and removes these comments.

Two other commands complete the set.  One is B<-cscp=string>, or
B<-closing-side-comment-prefix=string>, where string is the prefix used
before the name of the block type.  The default prefix, shown above, is
C<## end>.  It is best to stick to a single prefix to avoid mixing
prefixes in a file.  Otherwise, deleting closing side comments can
be difficult.

The last command is B<-cscl=string>, or
B<--closing-side-comment-list=string>, where C<string> is a list of
block types to be tagged with closing side comments.  By default, all
block types preceded by a keyword or label (such as C<if>, C<sub>, and so on) will
be tagged.  The B<-cscl> command changes the default list to be
any selected block types.  One block type which is not a keyword is
a statement label.  To request that a block with a label be included,
include a bare colon, C<:>, in the list of block types.  For example,
the following command requests that C<sub>'s, labels, C<BEGIN>, an
C<END> blocks be included in any addition or deletion operation:

   -cscl="sub : BEGIN END"

Adding and deleting of closing side comments take place only through
the commands C<-acsc> and B<-dcsc>.  The other two commands merely
define the prefix and the list of block types that will be considered
in any addition and deletion operations.
...
This command is equivalent to B<-olq> and B<-olc> and is included for
compatability with previous versions of perltidy.  The negation of this
also works, B<-noll> or B<--nooutdent-long-lines>, and is equivalent to
setting B<-nolq> and B<-nolc>.
...
=item Outdenting Control Keywords: B<-okw>,  B<--outdent-keywords>
...
Normally, B<-okw> would be used with B<-ola>.  The default is not to do
this.  
...
file:  (1) B<-log> will force a F<.LOG> file to be written, which
contains the path to the F<.perltidyrc> file, if any, and a listing of
its parameter settings.  (2) B<-opt> will force a F<.LOG> file to be
...
any command line options.  (3) the parameters in the F<.perltidyrc>
file can be ignored with the B<-npro> option.  (4) the
...
with alternative names, and invoke them with B<-pro=filename> on
the command line.  Then rename the desired file to F<.perltidyrc> 
when finished.
...
B<--dump-options> or B<-dop>  will write current option set to standard output and quit.
...
'n' to produce the negated form:
    ...
    q opt sbc sfs sts se st sob t tac tbc tp tsc tqw w
