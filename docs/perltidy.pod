...
Perltidy has a simple mechanism for displaying commented code.  When
B<-sbc> is used, a block comment with a special leading pattern
will be treated specially.
By default, this special pattern is two (or more) leading hash
symbols, C<##>, but this can be changed (see next item, B<-sbcp>).
Comments so identified  are treated as follows: (1) No blank line
will be inserted before such a comment, (2) If there is no leading space
on the line, then the comment will not be indented, and otherwise it
will, and (3) such a comment will never become a hanging side comment.
For example, assuming C<@month_of_year> is left-adjusted:
...
symbol, since this must only match comments.  Also, this string is used
in a perl regular expression, so it must not form an invalid regular
expression.  As a simple example, to identify all comments as static
block comments, one would use C<-sbcp=#>.  Please note that B<-sbcp>
merely defines the pattern used to identify static block comments; it
will not be used unless the switch B<-sbc> is set.
...
The current default is not to do this.  Use B<-nola> to not outdent
labels.

=item B<-ock>,  B<--outdent-control-keywords>

This command will cause certain leading control keywords to be outdented
by 2 spaces (or whatever B<-ci> has been set to), if possible.  The
keywords are C<redo>, C<next>, C<last>, C<goto>, and C<return>.  The
intention is to make these control keywords easier to see.  (To control
which keywords are outdented, see the next section).
...
Normally, B<-ock> would be used with B<-ola>.  The default is not to do
this.  

=item B<-okl=string>,  B<--outdent-keywords-list=string>

This command is a generalization of B<-ock>, and will cause any selected
leading keywords to be outdented by 2 spaces (or whatever B<-ci> has
been set to), if possible.  The parameter B<string> is a required list
of perl keywords.  

For example, the command C<-okl="next last redo goto"> will cause 
