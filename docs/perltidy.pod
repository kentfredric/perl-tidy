...
name).  For example, the flag to have perl check syntax using perl -c 
...
Please note that this flag only controls the invocation of perl for
checking.  Even if it is deactivated, perltidy will still do extensive
syntax checking on its own. 
...
Tabs are considered harmful, and will almost certainly lead to future
portability and maintenance problems, so the default and recommendation
is not to use them.  For those who prefer tabs, however, there are two
different options.  
...
This flag causes one leading tab character to be inserted for each
level of indentation.  Certain other features are incompatible with this
option, and if these are given, then a warning message will be issued
and this flag will be unset.  One example is the B<-lp> option.
...
closing side comments.  It will help prevent the loss of any existing side
comment which happens to have the closing side comment prefix.  It causes two
...
F<.perltidyrc> file and its contents to standard output and quit.
If no configuration file is found, a list of all filenames
tested will be written.

=item *

The parameters in the F<.perltidyrc>
file can be switched off with the B<-npro> option.  

=item *

The parameter B<-log> will force a F<.LOG> file to be written, which
contains the path to the F<.perltidyrc> file, if any, and a listing of
its parameter settings.  

=item *

The parameter B<-opt> will force a F<.LOG> file to be
