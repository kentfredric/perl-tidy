...
=item Vertical tightness of curly braces, parentheses, and square brackets.
...
control what we call vertical tightness.  Some important notes
regarding these parameters are:

=over 4

=item *

Most of these parameters require the variable indentation provided
by the B<-lp> flag, or otherwise they probably will not work properly.

=item *

Also, please note that these vertical tightness flags are merely hints
to the formatter, and it is not always possible for it to obey then.
Things which make it difficult or impossible include comments, blank
lines, and blocks of code within a list.  Also, these flags will often
be ignored for very small lists.

=back

The B<-pvt=n> or B<--paren-vertical-tightness=n> parameter controls if
and when a parenthesized container is "broken open".  The default is
B<n=0>, and causes most significant lists to be broken open.  A value
B<n=1> will cause a list to remain closed provided that this does not
produce two or more indentation steps on a single line (which can be
difficult to read).  A value of B<n=2> will try to keep all
parenthesized containers closed.

Here are some examples:

    # perltidy -lp -pvt=0 (default)
    @month_of_year = (
                       'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun',
                       'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'
    );
...
In the examples so far, the closing paren has behaved symmetrically with
the opening paren, either opening or closing.  This is the default, but
it can be controlled independently with the flag B<-pcvt=n> or
B<--paren-closing-vertical-tightness=n>.  The possible values for B<n>
are B<n=0>, always open up,  B<n=1> (default) do whatever the opening
paren does, and B<n=2> always stay closed.  Here are a few examples:

    # perltidy -lp -pvt=1 -pcvt=0
    @month_of_year = ( 'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun',
                       'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'
    );

    # perltidy -lp -pvt=0 -pcvt=2
    @month_of_year = (
                       'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun',
                       'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec' );
...
Similar parameters for non-code block braces are B<-bvt=n> or
B<--brace-vertical-tightness=n>, and B<-bcvt=n> or
B<--brace-closing-vertical-tightness=n>.

Likewise, the parameters for square brackets are B<-sbvt=n> or
B<--square-bracket-vertical-tightness=n>, and B<-sbcvt=n> or
B<--square-bracket-closing-vertical-tightness=n>.

Similar parameters for controlling code block braces are not currently
available.
...
See also B<-sbc> for a way to have some indented and some outdented
block comments.
...
A closing side comment was added for C<sub foo> in this case, but not
...
C<n=6>.  To illustrate, here is the above block processed with
C<-csci=2 -csc>:
...
The text after an C<else> block will be given the text of the
C<if> block which introduces it.  Additional text can be appended to
C<else> and C<elsif> closing side comments, as follows:
 
 n=0: else blocks get leading if block text (default)
 n=1: same as 0 plus:
        - elsif blocks also get leading if block text, and
        - else block gets terminal elsif block text if any,
      provided line length limit is not exceeded.
 n=2: same as 1 regardless of line length
...
'n' to produce the negated form:
    ...
    hsc html ibc icb icp lp log lal x lsl bl sbl okw ola oll
