...
Opening tokens are controlled by B<-vt=n>, or B<--vertical-tightness=n>, 
where
...
You must also use the B<-lp> flag to control the behavior of the opening
tokens. The reason is explained below.

=item *

Closing tokens are controlled by B<-vtc=n>, or
...
Parameters for controlling non-code block braces are B<-bvt=n> or
...
B<-pvt=n -bvt=n and sbvt=n>, and likewise B<-vtc=n> is an abbreviation
for B<-pvtc=n -bvtc=n and sbvtc=n>.
