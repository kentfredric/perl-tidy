...
If a closing block contains an existing side comment, then no closing
side comment will be created.
...
instead of '##' -- see below) to prevent perltidy from recognizing and 
changing the side comment.
...
where C<n> is the maximum number of columns of appended text (excluding
the name of the block).  Omitted text is indicated with C<...>.  The
default value is C<n=20>.  Tokens are never truncated, however, so
actual lengths may exceed this.  To illustrate, in the above example,
the appended text of the first block is C< ( !defined( $_[0] )...>.  The
existing limit of C<n=20> caused this text to be truncated, as indicated
by the C<...>.
