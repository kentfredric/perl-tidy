...
(where n is defined by the -i flag).  The default is to use ASCII space
...
The default and recommendation is B<-nt> (or B<--notabs>).
...
=item Sticky Block Comments

=over 4

=item B<-sbc>, B<--sticky-block-comments>
...
Please note that B<-sbcp> merely defines the pattern used to identify sticky
...
A closing side comment is a special comment which can be placed after the
closing brace of a code block.  They can be useful for code maintenance
and debugging.  The command B<-csc> (or B<-closing-side-comments>)
adds or refreshes closing side comments (but not if a different type
of side comment exists).
...
The command B<-ncsc> reverses this process and removes these comments.
If neither B<-csc> or B<-ncsc> are given, then perltidy will neither
add nor delete any closing side comments.  This is the default.

Three commands modify the behavior of B<-csc> and B<-ncsc>:

B<NOTE:> If you edit the closing side comments generated by perltidy,
your edits will probably be lost when you rerun perltidy with B<-csc>.
...
default prefix, shown above, is C<## end>.  This string will be added
...
comments.  By default, all block types preceded by a keyword or label (such as
C<if>, C<sub>, and so on) will be tagged.  The B<-cscl> command changes the
default list to be any selected block types.  One block type which is not a
keyword is a statement label.  To request that a block with a label be
included, include a bare colon, C<:>, in the list of block types.  For
example, the following command requests that C<sub>'s, labels, C<BEGIN>, and
...
place only through the commands C<-csc> or C<-ncsc>.  The other commands,
...
When B<-ssc> is used, a side comment with a static leading pattern,
which is C<##> by default, will be be spaced only a single space from
previous character, and it will not be vertically aligned with other
side comments.  This option is useful for controlling how commented code
is displayed. 

The default is to use B<-ssc>.  This may be deactivated with B<-nssc>.
...
sticky block comments; it will not be used unless the switch B<-ssc> is
...
'n' to produce the negated form:
    ...
    D anl asc aws bbb bbc bbs bli baa syn ce csc dac dbc dnl
...
This man page documents perltidy version 20011112.
