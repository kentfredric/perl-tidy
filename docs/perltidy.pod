...
B<-csc> (or B<-closing-side-comments>) adds or refreshes closing side
...
Three commands modify the behavior of B<-csc> and B<-dcsc>:
...
(such as C<if>, C<sub>, and so on) will be tagged.  The B<-cscl> command
changes the default list to be any selected block types.  To request
that a block with a label be included, include a bare colon, C<:>, in
the list of block types.  For example, the following command requests
that only C<sub>'s, labels, C<BEGIN>, and C<END> blocks be affected by
any B<-csc> or B<-dcsc> operation:
...
place only through the commands C<-csc> or C<-dcsc>.  The other commands,
if used, merely modify the behavior of these two commands.  

Also, note that if B<-csc> is used, it should continue to be used so that
...
This man page documents perltidy version 20011117.
