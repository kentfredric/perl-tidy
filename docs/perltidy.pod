...
C<else> and C<elsif> closing side comments, as follows:
 ...
 n=1: same as 0 plus, provided line length limit is not exceeded,
        - elsif blocks also get leading if block text, and
        - else block gets terminal elsif block text if any.
 n=2: same as 1 but not line length limit checks.
        
The latter options may be helpful when the cuddled-else style is used.
