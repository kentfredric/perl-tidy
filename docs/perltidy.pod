...
When the B<-lp> method is used for list indentation, excessive space
could cause an undesirable compression of a table against the right
...
with the parameter B<-mci=n>.  This parameter should seldom be required,
and the default is for unlimited indentation.
