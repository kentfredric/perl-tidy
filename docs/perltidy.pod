...
Execute perltidy on file F<somefile.pl>, with 3 columns for each
...
Output will be F<somefile.pl.tdy>. 
...
Execute perltidy on file F<somefile.pl> with all defaults except use
"cuddled elses" and a maximum line length of 72 columns (instead of the
default 80 columns).  

  perltidy -io somefile.pl

Execute perltidy on file F<somefile.pl> but restrict changes to "indentation
only", which means that existing line breaks and whitespace will be unchanged.
...
creates a F<.LOG> file, but by default it is deleted if there are no
errors.  Setting the B<-log> flag forces the log file to be saved.
...
Likewise, the parameter B<-bt=n> or B<--brace-tightness=n> controls the
...
And finally, the parameter B<-sbt=n> or B<--square-bracket-tightness>
...
=item List indentation using B<-gnu>, B<--gnu-style-indentation>
...
Use the B<-gnu> flag to add extra indentation to cause the data to begin
past the opening paren (or square bracket or curly brace).  With this
option set, the above list would become:
...
If the available line length of 80 columns ( or whatever is specified with
B<-l=n> ) does not permit this much space, perltidy will use less.

This option has no effect on code BLOCKS, which always use whatever is
specified with B<-i=n>.  Also, the existance of line breaks and/or
comments between the opening and closing parens may cause perltidy to
revert to its default method.

=item B<-mci=n>, B<--maximum-continuation-indentation=n>

When the B<-gnu> method is used for list indentation, excessive space
may cause an undesirable compression of a table against the right
margin, even if there is enough space to fit.  To prevent this, an upper
limit can be placed on the allowable increment of indentation spaces
with the parameter B<-mci=n>.  The default value of B<n> is 40.
...
To deactivate this feature, use B<--nohanging-side-comments>.  If block
...
default.  This is negated with B<-nbbs> or  B<--noblanks-before-subs>.

=item B<-bbb>,  B<--blanks-before-blocks>

A blank line may be introduced before blocks of coding delimited by
B<for>, B<foreach>, B<while>, B<until>, and B<if>.  This is the default.
...
However, the default style, B<-nbl> places the opening brace on a new
...
A conflict occurs if both B<-bl> and B<-bar> are specified, so don't do it.
...
When standard input is used, the log file, if saved, is F<perltidy.LOG>, and
any error file is F<perltidy.ERR>.  These are saved in the current working
directory.  If syntax checking is used, a temporary copy of the input file will
be created in the current working directory called F<perltidy.TMPI>, but it
...
This man page documents perltidy version 20010612.
