...
=head2 The command-line abbreviation feature is broken.

This error slipped into the current release, but it will be fixed 
in the next release.
...
=head2 Limitation parsing subroutine attributes and prototypes
...
name.  (Older versions of perl require the prototype to be on
the same line as the sub name, but perl 5.6 allows it to be on
a separate line).  For example, this would cause an error:
...
The workaround is to simply place the prototype and/or attribute on the same
line as the sub name, like this:
...
I have not encountered this problem in an actual script.  Fixing
it is low priority. 
