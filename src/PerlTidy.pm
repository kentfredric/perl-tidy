...
"skipping file: $input_file: Non-text (override with -f)\n";
            ...
            $source_object = PerlTidy::LineSource->new( $input_file, $rOpts );
...
sub new {
    ...
    if ( $rOpts->{'tidy-output'} ) {
        ( $fh, $output_file ) = PerlTidy::streamhandle( $output_file, 'w' );
        $output_file_open = 1;
    }
...
EOM
    ...
    foreach (@_) {
        if ( $PerlTidy::Tokenizer::is_keyword{$_} ) {
            $outdent_keyword{$_} = 1;
        }
        else {
    ...
    foreach $line ( 0 .. $max_line ) {
        ...
        foreach $i ( $ibeg .. $iend ) {
            ...
            # (within a line, since $i>1)
                ...
                if ( $token =~ /^(if|unless|and|or|eq|ne)$/ ) {
...
{    # begin set_comma_breakpoints_do
    ...
    # followed by a list
    @_ = qw(formline grep kill map printf sprintf push chmod join pack unshift);
